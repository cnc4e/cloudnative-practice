...
> 注意: 必ず以下を一通り確認してから作業してください。
>- [issueを対応する流れ](./practice-flow.md)
>- [コーディングルール](./coderule.md)
>
> 特に以下については忘れず対応してください。
>- ブランチを作成してからコーディングしマージする流れで進めて下さい。
>- tfファイル/yamlファイルは役割ごとに分割して下さい。
>- .gitignore に .terraform や *.tfstate を追加し、不要なディレクトリ・ファイルをGitで管理しないようにしてください。

>  注意: もし詰まってしまった場合、[example](../example)ディレクトリにサンプルコードを用意していますので参考にしてください。(exampleディレクトリに記載のコードは全Issue完了後の最終系のためご注意ください。)

以下タイトルのissueを対応してください。
